...
from single_model import BertConfig

batch_size = 1
bert_config_file = './bert_config.json'
ckpt = ''
output = './output/cct_cifar100.ckpt'
...
x = tf.placeholder(dtype=tf.float32, shape=(batch_size, 256, config.hidden_size), name='input')
...
with tf.Session() as sess:
    ...
    init = tf.global_variables_initializer()
    sess.run(init)
    input_x = np.load('./test_emb_res.npy')
    # value0 = sess.run(tf.get_default_graph().get_tensor_by_name('output_bias:0'))
    output_model = sess.run(model.get_nsp_output(), feed_dict={x: input_x})
    # sess.run(model.get_nsp_output(), feed_dict={x: sess.run(tf.ones([batch_size, 256, config.hidden_size],
    #                                                                dtype=tf.float32))})
